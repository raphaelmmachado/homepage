...
const createBookmarkElement = (bookmark) => {
  ...
  // SE LAYOUT FOR GRID
    ...
    element.innerHTML = `
      ...
      bookmark.title ?? bookmark.name
    }" class="w-7 h-7 object-contain mb-2 rounded-md"  />
...
export const applyLayout = () => {
  const gridSvg = layoutIconGrid.querySelector("svg");
  const listSvg = layoutIconList.querySelector("svg");
...
export const applyTheme = () => {
  const sunSvg = themeIconSun.querySelector("svg");
  const moonSvg = themeIconMoon.querySelector("svg");
